...
public abstract class MonkeyMotionEvent extends MonkeyEvent {
    ...
    public int injectEvent(IWindowManager iwm, IActivityManager iam, int verbose) {
        ...
        if(eventType == MonkeyEvent.EVENT_TYPE_TRACKBALL || eventType == MonkeyEvent.EVENT_TYPE_FLIP || eventType == MonkeyEvent.EVENT_TYPE_ROTATION || eventType == MonkeyEvent.EVENT_TYPE_KEY){
            if (this.isUninjectable(iam, null)){
        ...
        if ((verbose > 0 && !mIntermediateNote) || verbose > 1) {
            ...
            switch (me.getActionMasked()) {
                case MotionEvent.ACTION_DOWN:
                    if(this.isUninjectable(iam, me)){
                        System.out.println("this event is uninjectable...");
                        return MonkeyEvent.INJECT_FAIL;
                    }
