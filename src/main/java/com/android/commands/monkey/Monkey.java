...
import com.fkinh.monkey.FireFightMonitor;
...
public class Monkey {
    ...
    // flyme required 400 ms to sleep
    long mThrottle = 400;
    ...
    private class ActivityController extends IActivityController.Stub {
        ...
        public boolean activityResuming(String pkg) {
            ...
            boolean allow = checkEnteringPackage(pkg);
            if (!allow) {
                if (mVerbose > 0) {
                    System.out.println("    // " + ("Rejecting") + " resume of package " + pkg);
                }
            }
            currentPackage = pkg;
            StrictMode.setThreadPolicy(savedPolicy);
    ...
    private int run(String[] args) {
        // set accessibility event monitor
        FireFightMonitor monitor =  FireFightMonitor.getInstance();
        monitor.run();
        ...
        if (mMainCategories.size() == 0) {
            ...
            // add leanback for flyme tv
//            mMainCategories.add(Intent.CATEGORY_DEFAULT);
//            mMainCategories.add(Intent.CATEGORY_LEANBACK_LAUNCHER);
//            mMainCategories.add(Intent.CATEGORY_LEANBACK_SETTINGS);
        }
...
//            mAm.setActivityController(new ActivityController());
    ...
    private void showUsage() {
        ...
        usage.append("************************************\n");
        usage.append("Scriptkeeper monkey is watching you.\n");
        usage.append("************************************\n");
        usage.append("\n");
